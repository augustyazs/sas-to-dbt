...
def _render_architect_detail(plan):
    with st.expander("Architect", expanded=False):
        t1, t2 = st.tabs(["Planned Models", "Edge Cases"])
        with t1:
...
def _render_fixer_detail(final_state):
    if final_state.get("review_count", 0):
        with st.expander("Fixer", expanded=False):
            st.info(f"Fixer ran {final_state['review_count']} time(s). "
                    f"See Pipeline Logs → fixer_raw_* for details.")


def _run_pipeline_and_store(sas_code, mapping_raw):
    """Run pipeline, store everything in session state."""
    ...
    # pipeline_steps already populated by runner.py during the run
...
<style>
    ...
    div[data-testid="stMetric"] {
        background:#f8f9fa; padding:12px; border-radius:8px; border:1px solid #e0e0e0;
    }
    ...
    .human-review-alert {
        background:#fef3c7; border:2px solid #f59e0b; border-radius:8px; padding:16px; margin:12px 0;
    }
...
<div class="zs-header">
    ...
    <div><h1>HPP Capabilities</h1><p>SAS to dbt Agentic Code Migration</p></div>
...
with st.sidebar:
    ...
    output_branch = st.selectbox("Branch", ["feature/sas-migration","develop","main"], key="output_branch")
...
with tab_select:
    ...
    with c1:
        sf = get_sas_files()
        if sf:
            sel = st.selectbox("Select SAS Script", ["-- select --"] + sf, key="sas_select")
            if sel != "-- select --":
    ...
    with c2:
        mf = get_mapping_files()
        if mf:
            sel = st.selectbox("Select Column Mapping", ["-- select --"] + mf, key="map_select")
            if sel != "-- select --":
...
# ── EXECUTE (only on button click) ───────────────────────────────────────────
if run_clicked and can_run:
    _run_pipeline_and_store(sas_code, mapping_raw)


# ── RENDER OUTPUTS (from session state — survives all reruns) ─────────────────
...
if final_state is None and not st.session_state.pipeline_steps:
    st.stop()

# Two-column layout — only reached if pipeline has run at least once
st.divider()
col_main, col_progress = st.columns([3, 1])

# ── RIGHT: Pipeline Progress (rendered from session state) ────────────────────
with col_progress:
